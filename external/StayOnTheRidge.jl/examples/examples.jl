...
# characterization at [0,1]²: convex-concave (bilinear)
...
# characterization at [-1,1]²: nonconvex-nonconcave
...
# characterization at [-1,1]²: convex-nonconcave
...
# characterization at [-1,1]²: convex-concave
...
# characterization at [-1,1]x[-2π,2π]: convex-nonconcave
...
# characterization at [-1,1]²: nonconvex-concave
...
# characterization at [0,1]²: nonconvex-nonconcave
...
# Example 15 - Bilinearly-coupled Minimax” - example 3 from https://proceedings.neurips.cc/paper_files/paper/2023/file/a961dea42c23c3c0d01b79918701fb6e-Paper-Conference.pdf
# domain: [-4,4]²
# solution given by this implementation: (-0.0005,0.0001)
# solutions presented in the paper: (0,0)
# characterization at [-4,4]²: nonconvex-nonconcave
function example15(sym::Bool)
    n = 2
    min_coords = [1]
    γ = 1e-4
    ϵ = 1e-1
    domain = Hyperrectangle([[-4,4],[-4,4]])
    if sym
        x = [Symbolics.variable(:x,i) for i in 1:n]
        f = (x[1]+1)*(x[1]-1)*(x[1]+3)*(x[1]-3) + 11*x[1]*x[2] - (x[2]+1)*(x[2]-1)*(x[2]+3)*(x[2]-3)
        return Config_sym(f, x, n, min_coords, γ, ϵ, domain)
    else
        f_fd(x) = (x[1]+1)*(x[1]-1)*(x[1]+3)*(x[1]-3) + 11*x[1]*x[2] - (x[2]+1)*(x[2]-1)*(x[2]+3)*(x[2]-3)
        return Config_FD(f_fd, n, min_coords, γ, ϵ, domain)
    end
end

# Example 16 - example 2 from https://proceedings.neurips.cc/paper_files/paper/2023/file/a961dea42c23c3c0d01b79918701fb6e-Paper-Conference.pdf
# domain: [-1,1]²
# solution given by this implementation: (0,0)
# solutions presented in the paper: (0,0)
# characterization at [-1,1]²: nonconvex-nonconcave
function example16(sym::Bool)
    n = 2
    min_coords = [1]
    γ = 1e-3
    ϵ = 1e-2
    ...
    if sym
        x = [Symbolics.variable(:x,i) for i in 1:n]
        f = x[1]^2+3*sin(x[1])^2*sin(x[2])^2-4*x[2]^2-10*sin(x[2])^2
        return Config_sym(f, x, n, min_coords, γ, ϵ, domain)
    else
        f_fd(x) = x[1]^2+3*sin(x[1])^2*sin(x[2])^2-4*x[2]^2-10*sin(x[2])^2
        return Config_FD(f_fd, n, min_coords, γ, ϵ, domain)
    end
end

# Example 17 - example 17 from http://proceedings.mlr.press/v89/adolphs19a/adolphs19a.pdf
# domain: [-1,1]²
# solution given by this implementation: (0,0)
# solutions presented in the paper: (0,0)
# characterization at [-1,1]²: convex-nonconcave
function example17(sym::Bool)
    n = 2
    min_coords = [1]
    γ = 1e-3
    ϵ = 1e-2
    ...
    if sym
        x = [Symbolics.variable(:x,i) for i in 1:n]
        f = 2*x[1]^2+x[2]^2+4*x[1]*x[2]+4*x[2]^3/3-x[2]^4/4
        return Config_sym(f, x, n, min_coords, γ, ϵ, domain)
    else
        f_fd(x) = 2*x[1]^2+x[2]^2+4*x[1]*x[2]+4*x[2]^3/3-x[2]^4/4
        return Config_FD(f_fd, n, min_coords, γ, ϵ, domain)
    end
end

# Example 18 - example 5.2 from https://proceedings.mlr.press/v139/hsieh21a.html
# domain: [0,1]²
# solution given by this implementation: (0.0787,0.4102)
# solutions presented in the paper: (0.08,0.4)
# characterization at [0,1]²:
function example18(sym::Bool)
    n = 2
    min_coords = [1]    
    γ = 1e-3
    ϵ = 1e-4
    ...
    if sym
        x = [Symbolics.variable(:x,i) for i in 1:n]
        f = x[1]*(x[2]-0.45)+1/4*x[1]^2-1/2*x[1]^4+1/6*x[1]^6-(1/4*x[2]^2-1/2*x[2]^4+1/6*x[2]^6)
        return Config_sym(f, x, n, min_coords, γ, ϵ, domain)
    else
        f_fd(x) = x[1]*(x[2]-0.45)+1/4*x[1]^2-1/2*x[1]^4+1/6*x[1]^6-(1/4*x[2]^2-1/2*x[2]^4+1/6*x[2]^6)
        return Config_FD(f_fd, n, min_coords, γ, ϵ, domain)
    end
end

# Example 19 - classical example with no NE: (x₁-x₂)²
# domain: [0,1]²
# solution given by this implementation: (0,0)
# characterization at [0,1]²:
function example19(sym::Bool)
    n = 2
    min_coords = [1]    
    γ = 1e-3
    ϵ = 1e-4
...
is_solution_VI(min_max1, config1, config1.domain)
...
is_solution_VI(min_max2, config2, config2.domain)
...
is_solution_VI(min_max3, config3, config3.domain)
...
is_solution_VI(min_max4, config4, config4.domain)
...
is_solution_VI(min_max5, config5, config5.domain)
...
is_solution_VI(min_max6, config6, config6.domain)
...
is_solution_VI(min_max7, config7, config7.domain)
...
is_solution_VI(min_max8, config8, config8.domain)
...
is_solution_VI(min_max9, config9, config9.domain)
...
is_solution_VI(min_max10, config10, config10.domain)
...
is_solution_VI(min_max11, config11, config11.domain)
...
is_solution_VI(min_max12, config12, config12.domain)
...
is_solution_VI(min_max13, config13, config13.domain)
...
is_solution_VI(min_max14, config14, config14.domain)
...
config15 = example15(false);
elapsed15 = @elapsed min_max15, trajectory15, m15, k15 = run_dynamics(config15)
pretty_print(min_max15, elapsed15, m15, k15)
is_solution_VI(min_max15, config15, config15.domain)
plot_trajectory2D(min_max15, trajectory15, config15.domain)
plot_contour2D(min_max15, config15.f, config15.domain)
plot_surface(min_max15, config15.f, config15.domain)

config16 = example16(false);
elapsed16 = @elapsed min_max16, trajectory16, m16, k16 = run_dynamics(config16)
pretty_print(min_max16, elapsed16, m16, k16)
is_solution_VI(min_max16, config16, config16.domain)
plot_trajectory2D(min_max16, trajectory16, config16.domain)
plot_contour2D(min_max16, config16.f, config16.domain)
plot_surface(min_max16, config16.f, config16.domain)

config18 = example18(true);
elapsed18 = @elapsed min_max18, trajectory18, m18, k18 = run_dynamics(config18)
pretty_print(min_max18, elapsed18, m18, k18)
is_solution_VI(min_max18, config18, config18.domain)
plot_trajectory2D(min_max18, trajectory18, config18.domain)
plot_contour2D(min_max18, config18.f, config18.domain)
plot_surface(min_max18, config18.f, config18.domain)

config19 = example19(true);
elapsed19 = @elapsed min_max19, trajectory19, m19, k19 = run_dynamics(config19)
pretty_print(min_max19, elapsed19, m19, k19)
is_solution_VI(min_max19, config19, config19.domain)
plot_trajectory2D(min_max19, trajectory19, config19.domain)
plot_contour2D(min_max19, config19.f, config19.domain)
plot_surface(min_max19, config19.f, config19.domain)