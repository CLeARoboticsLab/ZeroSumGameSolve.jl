...
function plot_contour2D(min_max, f, domain::Domain)
    ...
    bounds = get_bounds(domain)
    contourf(rs[1], rs[2], f_plot, levels=20, xlims = [bounds[1], bounds[2]], ylims = [bounds[3], bounds[4]]) # size=(800, 600), dpi=300
...
function plot_surface(min_max, f, domain::Domain)
    ...
    surface(rs[1], rs[2], zs, legend=false) # size=(800, 600), dpi=300
    # zticks!(0.25:0.25:1)
    # xlabel!("θ")
    # ylabel!("ω")
