...
struct Config_sym <: Config
    ...
    V_origin::Vector{Function} # modified gradient of f on the [0,1]ⁿ
    ...
    function Config_sym(f_expr::Num, x::Vector{Num}, n::Int, min_coords::Vector{Int},
        ...
        V_origin_expr = prepare_V(f_expr, x, min_coords, Default())
        V_origin = [eval(build_function(V_origin_expr[i], x)) for i in 1:n]
        ...
        return new(f, V, V_origin, J, x, n, min_coords, γ, ϵ, domain, grad_f, hess_mat_f)
    end
end
